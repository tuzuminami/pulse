...
Only release from a green `main` commit through a pull request. Create and push an annotated matching `v<package-version>` tag from the current `main` commit first, then create its draft release with `gh release create v<version> --verify-tag --draft`, and finally run the release-evidence workflow against that draft. A draft release alone creates no Git ref, so it cannot be used as CI's release source. The workflow rejects a missing or lightweight tag, a non-draft release, and a tag whose commit differs from current `main`. It creates the npm tarball, reproducible CycloneDX SBOM, public synthetic evaluation bundle, and release-decision manifest. The CI artifact is retained for 90 days; after publishing the completed draft with immutable releases enabled, the GitHub Release assets are the long-term canonical audit record. The workflow signs package provenance, the SBOM, and the release-decision manifest through GitHub Artifact Attestations. Do not rewrite a public tag; publish a corrective patch release instead.
